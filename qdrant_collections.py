...
DIM_REID = 1000          # TorchReID OSNet embedding
...
VECTOR_PARAMS_1000 = VectorParams(size=DIM_REID, distance=Distance.COSINE)
VECTOR_PARAMS_768 = VectorParams(size=DIM_MULTI, distance=Distance.COSINE)

# --- 2. Named Vector Definitions ---

# Person Tracks: face_vec (512D), reid_vec (1000D), multi_vec (768D for CLIP)
PERSON_TRACKS_VECTORS = {
    "face_vec": VECTOR_PARAMS_512,  # Face embedding from InsightFace (512D)
    "reid_vec": VECTOR_PARAMS_1000, # Body ReID embedding from TorchReID (1000D)
...
def create_qdrant_schema(client: QdrantClient):
    ...
    try:
        ...
        if PERSON_TRACKS_COLLECTION in collection_names:
            print(f"✅ Collection '{PERSON_TRACKS_COLLECTION}' already exists, skipping creation")
        else:
            client.create_collection(
                collection_name=PERSON_TRACKS_COLLECTION,
                vectors_config=PERSON_TRACKS_VECTORS
            )
            print(f"✅ Collection '{PERSON_TRACKS_COLLECTION}' created with vectors: face_vec (512D), reid_vec (1000D), multi_vec (768D)")
            person_created = True
    ...
    try:
        if OBJECT_TRACKS_COLLECTION in collection_names:
            print(f"✅ Collection '{OBJECT_TRACKS_COLLECTION}' already exists, skipping creation")
        else:
            client.create_collection(
    ...
    if person_created:
        ...
        print(f"⏭️ Skipping indexing for existing {PERSON_TRACKS_COLLECTION}")
    ...
    if object_created:
        ...
        print(f"⏭️ Skipping indexing for existing {OBJECT_TRACKS_COLLECTION}")
