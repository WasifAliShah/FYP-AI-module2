...
def get_upscale_factor(face_size):
    """Determine upscale factor based on face size."""
    if face_size < 30:
        return 4
    elif face_size < 45:
        return 3
...
def extract_face_embedding(person_crop, face_boxes_frame, frame, fa, w, h):
    """Extract face embedding from person crop using multiple methods."""
    face_emb = None
    face_size = 0
    ...
    for fb in face_boxes_frame:
        if box_inside(fb, (0, 0, person_crop.shape[1], person_crop.shape[0])) or iou(fb, (0, 0, person_crop.shape[1], person_crop.shape[0])) > 0.1:
            matched_face = fb
            break
    ...
    if matched_face is not None:
        ...
        upscale_factor = get_upscale_factor(face_size)
        if upscale_factor > 1:
            ...
            if faces and len(faces) > 0:
                ...
                if upscale_factor > 1:
                    ...
                    if hasattr(f, 'bbox') and f.bbox is not None:
                        ...
                        bx2 = min(person_crop.shape[1], int(f.bbox[2])); by2 = min(person_crop.shape[0], int(f.bbox[3]))
                        if bx2 > bx1 and by2 > by1:
                            face_region = person_crop[by1:by2, bx1:bx2]
                ...
                x1_exp = max(0, 0 - expand)
                y1_exp = max(0, 0 - expand)
                x2_exp = min(w, person_crop.shape[1] + expand)
                y2_exp = min(h, person_crop.shape[0] + expand)
...
def insert_tracklet_to_qdrant(client, tracklet, video_id=1, segment_id=None, frame_rate=30.0):
    ...
    Args:
        ...
        tracklet: Tracklet object with avg face/reid embeddings
    ...
    try:
        ...
        reid_vec = reid_avg.tolist() if reid_avg is not None and isinstance(reid_avg, np.ndarray) else (list(reid_avg) if reid_avg is not None else [0.0] * 1000)
        ...
        # Build vectors dict for NamedVectors (per spec: face_vec 512D, reid_vec 1000D, multi_vec 768D)
        vectors = {
            "face_vec": face_vec,          # 512D (InsightFace)
            "reid_vec": reid_vec,          # 1000D (TorchReID)
...
def process_non_detection_frame(frame_idx, h, w, tracklets, object_tracklets, byte_tracker, byte_tracker_objects, DETECT_EVERY_N_FRAMES):
    """Process frames that are not detection frames: update ByteTrack with empty detections."""
...
def process_detection_frame(frame, frame_idx, h, w, yolo_person, yolo_face, yolo_objects, OBJECT_CLASSES, DETECT_EVERY_N_FRAMES, byte_tracker, byte_tracker_objects, tracklets, object_tracklets, next_tid, next_obj_id, OBJ_TRACK_MAX_AGE, OBJ_IOU_THRESHOLD, face_boxes_frame):
    """Process detection frames: run YOLO detections, update ByteTrack, extract embeddings."""
    ...
    return tracked_objects, person_boxes, detected_objects, face_boxes_frame
...
while True:
    ...
    # Process non-detection frames
    if frame_idx % DETECT_EVERY_N_FRAMES != 0:
        process_non_detection_frame(frame_idx, h, w, tracklets, object_tracklets, byte_tracker, byte_tracker_objects, DETECT_EVERY_N_FRAMES)
        continue  # Skip to next frame

    # Process detection frame
    tracked_objects, person_boxes, detected_objects, face_boxes_frame = process_detection_frame(
        frame, frame_idx, h, w, yolo_person, yolo_face, yolo_objects, OBJECT_CLASSES, DETECT_EVERY_N_FRAMES,
        byte_tracker, byte_tracker_objects, tracklets, object_tracklets, next_tid, next_obj_id, OBJ_TRACK_MAX_AGE, OBJ_IOU_THRESHOLD, face_boxes_frame
    )
    ...
    if USE_BYTETRACK and len(tracked_objects) > 0:
        ...
        for track in tracked_objects:
            ...
            # Extract face and ReID embeddings
            face_emb, face_size = extract_face_embedding(crop_person, face_boxes_frame, frame, fa, w, h)
            reid_emb = reid_encode(crop_person) if USE_REID_VERIFICATION else None
...
# Face and ReID extraction
    ...
    for tid, t in list(tracklets.items()):
        ...
        if not t.processed and (len(t.face_embs)+len(t.reid_embs)) >= AGGREGATION_FRAMES:
            ...
            # Insert to Qdrant for all tracklets with sufficient embeddings
            if client and face_avg is not None:
