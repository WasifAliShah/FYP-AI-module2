...
class _DeepSortByteTrackCompat:
    ...
    def __init__(self, **kwargs):
        # Use internal embedder so we don't need external appearance features
        self.ds = DeepSort(
            embedder='torchreid',
            embedder_gpu=False,
            max_age=kwargs.get('track_buffer', 30),
            n_init=1,
            max_iou_distance=0.9,  # looser gating for fast motion
...
if USE_DEEPSORT:
    try:
        ...
        cap_temp.release()
        byte_tracker = _DeepSortByteTrackCompat(track_buffer=60)
        print(f"✓ DeepSORT initialized for persons (FPS: {fps:.1f})")
        # For small objects like phones, keep tracks alive longer
        byte_tracker_objects = _DeepSortByteTrackCompat(track_buffer=140)
        print(f"✓ DeepSORT initialized for objects (FPS: {fps:.1f})")
    except Exception as e:
...
def reassign_small_object_ids(object_tracklets, class_keywords, iou_thresh=0.4, max_center_dist=100):
        ...
        for i in range(len(objects)):
            ...
            for j in range(i+1, len(objects)):
                ...
                should_merge = (
                    ...
                    (iou_val >= 0.2 and dist_current <= max_center_dist * 1.5)  # some overlap + nearby
                )
...
while True:
    ...
    if USE_BYTETRACK and byte_tracker_objects is not None and not is_object_detection_frame:
            try:
                ...
                for oid, obj_track in object_tracklets.items():
                    ...
                    is_backpack = any(keyword in obj_track.class_name.lower() for keyword in ["backpack", "bag", "handbag"])
                    
                    if is_phone or is_backpack:
                        # Allow double age for phones and backpacks
    ...
    if is_person_detection_frame:
        ...
        if USE_BYTETRACK and len(tracked_objects) > 0:
            ...
            for box in person_boxes:
                ...
                # ---- Carried objects + context-aware CLIP
                carried_obj_bboxes = []
                if detected_objects:
                    for class_name, obj_bbox, obj_conf in detected_objects:
    ...
    # Run reassignment multiple times for better convergence (phones and backpacks)
    for _ in range(3):
        # Phones: small and fast-moving, need aggressive matching
        reassign_small_object_ids(object_tracklets, 
                                  class_keywords=["phone", "cell phone", "mobile phone"], 
                                  iou_thresh=0.4, max_center_dist=100)
        # Backpacks: larger but also prone to ID churn, use slightly more relaxed params
        reassign_small_object_ids(object_tracklets, 
                                  class_keywords=["backpack", "bag", "handbag"], 
                                  iou_thresh=0.45, max_center_dist=120)
