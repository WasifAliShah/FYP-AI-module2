...
try:
    schema_ok = qdrant_collections.create_qdrant_schema(
        client,
    ...
    if not schema_ok:
        print(
            ...
            "In POST-PROCESSING mode, run once with REALTIME_FACE_COMPARISON=true"
        )
        sys.exit(1)

    print(
        "✅ Qdrant schema initialized for REAL-TIME mode"
        if REALTIME_FACE_COMPARISON
        else "✅ Qdrant schema validated for POST-PROCESSING mode"
    )
...
def find_similar_object_tracks(client, obj_embedding, video_id, object_type=None, object_color=None, threshold=0.85, top_k=5):
    """
    Search for similar object tracks in Qdrant using CLIP embeddings.
    
    Args:
        client: QdrantClient instance
        obj_embedding: 512D CLIP embedding vector for the object
        video_id: Video ID to filter by (required)
        object_type: Object type/class to filter by (optional, e.g., "cell phone")
        object_color: Object color to filter by (optional)
        threshold: Similarity threshold (0-1) to consider a match
        top_k: Maximum number of results to return
    
    Returns:
        list: [(point_id, track_id, similarity_score, payload), ...] sorted by similarity desc
    """
    if not client:
        return []
    ...
    try:
        ...
        if object_color is not None:
            must_conditions.append(
                FieldCondition(
                    key="object_color",
                    match=MatchValue(value=object_color)
                )
            )
        
        search_filter = Filter(must=must_conditions) if must_conditions else None
        
        # Search using object_vec vector
        try:
            search_results = client.query_points(
                collection_name="object_tracks",
                query=obj_embedding.tolist() if hasattr(obj_embedding, 'tolist') else list(obj_embedding),
                using="object_vec",
            ...
            try:
                search_results = client.search(
                    collection_name="object_tracks",
                    query_vector=("object_vec", obj_embedding.tolist() if hasattr(obj_embedding, 'tolist') else list(obj_embedding)),
                    query_filter=search_filter,
                    limit=top_k,
                    with_payload=True
                )
                ...
                search_results = client.search(
                    collection_name="object_tracks",
                    query_vector=obj_embedding.tolist() if hasattr(obj_embedding, 'tolist') else list(obj_embedding),
                    query_filter=search_filter,
                    limit=top_k,
                    with_payload=True,
        ...
        # Filter results by threshold and extract relevant info
        matches = []
        for result in search_results:
            similarity = result.score  # Qdrant returns cosine similarity (0-1)
            if similarity >= threshold:
                point_id = result.id  # Get the point ID directly
                track_id = result.payload.get("track_id", "unknown")
                payload = result.payload
                matches.append((point_id, track_id, similarity, payload))  # Include point_id
        
        return matches
        
    except Exception as e:
        print(f"⚠ Error searching for similar object tracks: {e}")
        return []
...
def insert_object_track_to_qdrant(client, obj_track, video_id=1, segment_id=None, frame_rate=30.0):
    ...
    First searches for similar existing tracks. If found:
    - If time difference > 1 second: updates existing track with reappearance info
    - If time difference <= 1 second: merges details without marking as reappearance
    ...
    Returns:
        bool: True if insertion/update succeeded, False otherwise
    """
    # Skip inserts in POST-PROCESSING mode
    try:
        if not REALTIME_FACE_COMPARISON:
    ...
    try:
        ...
        # Get CLIP embedding for similarity search
        avg_clip_emb = obj_track.avg_clip()
        if avg_clip_emb is None:
            avg_clip_emb = np.zeros(512, dtype=np.float32)
        
        # Search for similar tracks BEFORE inserting
        similar_tracks = find_similar_object_tracks(
            client,
            avg_clip_emb,
            video_id=video_id,
            object_type=obj_track.class_name,
            object_color=obj_track.get_dominant_color(),
            threshold=0.85,
            top_k=5
        )
        
        # Build base payload for object_tracks collection
        payload = {
            "video_id": video_id,
        ...
        if similar_tracks:
            # Use the top match and its point_id directly
            existing_point_id, best_match_track_id, best_similarity, best_payload = similar_tracks[0]
            existing_end_time_sec_str = best_payload.get("end_time", "00:00:00.000")
            
            # Parse existing end time to calculate time difference
            try:
                parts = existing_end_time_sec_str.split(":")
                h = int(parts[0])
                m = int(parts[1])
                s_ms = parts[2].split(".")
                s = int(s_ms[0])
            ...
            if time_diff > 1.0:
                print(f"🔄 Object {obj_track.class_name} ID {obj_track.id} → REAPPEARANCE (gap: {time_diff:.2f}s)")
                print(f"   Previous ended: {existing_end_time_sec_str} | New starts: {start_time_str}")
                ...
                # Calculate duration of this appearance session
                ...
                print(f"🔗 Object {obj_track.class_name} ID {obj_track.id} → MERGED (gap: {time_diff:.2f}s)")
                print(f"   Previous ended: {existing_end_time_sec_str} | New starts: {start_time_str}")
                merged_payload["last_appearance_time"] = end_time_str
            
            merged_payload["merged_track_ids"] = merged_payload.get("merged_track_ids", []) + [obj_track.id]
            merged_payload["last_merge_time"] = datetime.now().isoformat()
            ...
            print(f"✓ UPDATED existing record (Track {best_match_track_id}) with merged data (similarity: {best_similarity:.4f})")
            return True
        else:
            # No similar track found, insert as new
            print(f"✨ Object {obj_track.id} ({obj_track.class_name}) is NEW, inserting...")
            point_id = str(uuid.uuid4())
            
            # Initialize unified schema for new objects
            ...
            print(f"✓ Inserted NEW object to Qdrant (Track {obj_track.id}, Class: {obj_track.class_name})")
            return True
